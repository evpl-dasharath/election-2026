    ...
    <header className="bg-ink text-white shrink-0 sticky top-0 z-50 border-b border-white/10">
      ...
      <div className="px-3 md:px-6 py-2 md:py-0 min-h-[3rem] md:h-12 flex flex-wrap md:flex-nowrap items-center justify-between gap-y-2 gap-x-4">
        ...
        <div className="flex items-center gap-2 md:gap-2.5 shrink-0 md:w-[200px]">
          <button
            className="font-serif text-[15px] md:text-[17px] tracking-tight whitespace-nowrap cursor-pointer hover:text-white/85 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50 rounded px-1 -ml-1 text-left"
            onClick={() => navigate('/')}
          ...
          {allPills.map(({ id, won, lead, share, color }) => {
            ...
            return (
              <Component
                ...
                className={isClickable ? "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50 transition-shadow" : ""}
                style={{
          ...
          <div className="flex flex-wrap justify-end gap-x-2 gap-y-1 md:gap-x-4">
            {navItems.map(({ label, active, onClick }) => (
              <button
                key={label}
                onClick={onClick}
                className={`text-[10px] md:text-[11px] font-medium cursor-pointer hover:text-white/80 transition-colors whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50 rounded px-1.5 py-0.5 -mr-1.5 ${active ? 'text-white' : 'text-white/45'}`}
              >
                {label}
              </button>
            ))}
