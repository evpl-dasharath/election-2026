...
def run_single_cycle(ac_numbers, base_url, state_code, stdout):
    ...
    stdout.write(f"  Matched: {matched} | Partial: {partial} | Pending: {pending} | Failed: {failed} / {len(ac_numbers)}")
    return results


class Command(BaseCommand):
    help = "Continuous ECI scrape loop"
    ...
    def handle(self, *args, **options):
        ...
        ac_numbers = [single_ac] if single_ac else list(
            Constituency.objects.values_list("number", flat=True).order_by("number")
        )
        label = f"AC {single_ac}" if single_ac else f"All {len(ac_numbers)} constituencies"
        ...
        if not run_forever:
            # Single AC, no loop — run once and exit
        ...
        try:
            while True:
                ...
                run_single_cycle(ac_numbers, base_url, state_code, self.stdout)
                elapsed = time.time() - start
                self.stdout.write(self.style.SUCCESS(f"  Cycle {cycle} done in {elapsed:.0f}s"))
